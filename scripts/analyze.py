...
def get_syntactic_chunks(output, activations, model_id, tokenizer, response_start_string="<think>"):
...
def benchmark_output_to_probe_scores(benchmark_output, selected_layer, layers, model_id, scaler, pca, probe):
    ## a. Get activations.
    ...
    results = run_activations.get_labeled_activations(model, tokenizer, benchmark_output, len(layers))
    results = run_activations.get_activation_by_index(results, selected_layer)
